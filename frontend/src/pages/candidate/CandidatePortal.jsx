...
export default function CandidatePortal() {
  ...
  useEffect(() => {
    async function loadData() {
      setLoading(true);
      try {
        const [ivs, props] = await Promise.all([
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-2xl font-extrabold text-slate-900 mt-1">
                Welcome, {user?.name || 'Rahul Mehta'}
              </h1>
              <p className="text-xs text-slate-500">
                Application: Senior Backend Engineer • Timezone: {user?.timezone || 'Asia/Kolkata (IST)'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {proposals.length > 0 && (
              <button
                ...
                className="btn-primary text-xs py-2 px-3.5 flex items-center gap-1.5 animate-pulse"
              >
                <Sparkles className="h-4 w-4" /> {proposals.length} Slots Awaiting Choice
              </button>
            )}
          </div>
        </div>
                ...
                <div className="p-4 rounded-xl bg-gradient-to-tr from-sky-50 to-blue-50/40 border border-sky-200 text-xs">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-extrabold text-sm text-slate-900">
                      {upcomingInterview.request?.roundName || 'Technical Round 1'}
                    </span>
                  ...
                  <div className="mt-4 pt-3 border-t border-sky-200/60 flex items-center justify-between">
                    <span className="text-[11px] text-slate-500 font-medium">
                      Panel: {upcomingInterview.panel?.map((p) => p.interviewer?.user?.name).join(', ') || 'Assigned Interviewer'}
                    </span>
                    <a
